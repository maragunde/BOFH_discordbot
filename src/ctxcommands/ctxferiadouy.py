...
async def feriadouyfunctx(ctx):
    today = datetime.date.today()
    anio = datetime.date.today().year
    
    # Llama a la API
    response = requests.get(f"https://date.nager.at/api/v3/PublicHolidays/{anio}/UY")
    try:
        if response.status_code == 200:
            ...
            print(FechaActual)
            print(f"Comando FeriadoUY ejecutado")
            
            # Se crea el mensaje con los feriados
            message = "Próximos feriados en Uruguay:\n"
