...
async def feriadoARfun(interaction):
    today = datetime.date.today()
    anio = datetime.date.today().year
    
    # Llama a la API
    response = requests.get(f"https://date.nager.at/api/v3/PublicHolidays/{anio}/AR")
    try:
        if response.status_code == 200:
            ...
            print(FechaActual)
            print(f"Comando FeriadoAR ejecutado por {interaction.user}")
            
            # Se crea el embed con los feriados
            embed = Embed(title=f"Proximos feriados en Argentina", description=f"A pedido de {interaction.user}", color = discord.Color.green())
