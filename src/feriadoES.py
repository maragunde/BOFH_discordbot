...
async def feriadoESfun(interaction):
    today = datetime.date.today()
    anio = datetime.date.today().year
    
    #Llama a la API
    response = requests.get(f"https://date.nager.at/api/v3/PublicHolidays/{anio}/ES")
    try:
        if response.status_code == 200:
          ...
          # Log
           ...
           # Se crea el embed con los feriados
            embed = Embed(title=f"Proximos feriados en España", description=f"A pedido de {interaction.user}", color = discord.Color.green())
